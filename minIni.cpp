...
static TCHAR *ini_strncpy(TCHAR *dest, const TCHAR *source, size_t maxlen,
	...
	assert(
			(dest < source || (dest == source && option != QUOTE_ENQUOTE))
...
long ini_getl(const TCHAR *Section, const TCHAR *Key, long DefValue,
	...
	int len = ini_gets(Section, Key, __T(""), LocalBuffer,
...
int ini_browse(INI_CALLBACK Callback, void *UserData, const TCHAR *Filename) {
	...
	for (;;) {
		...
		*ep++ = '\0'; /* split the key from the value */
		striptrailing(sp);
		ini_strncpy(LocalBuffer + lenSec, sp, INI_BUFFERSIZE - lenSec,
				QUOTE_NONE);
...
static enum quote_option check_enquote(const TCHAR *Value) {
	...
	for (p = Value; *p != '\0' && *p != '"' && *p != ';' && *p != '#'; p++)
		/* nothing */;
	return (*p != '\0' || (p > Value && *(p - 1) == ' ')) ?
			QUOTE_ENQUOTE : QUOTE_NONE;
}
